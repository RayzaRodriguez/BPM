...
// se considera el ancho y altura y numero de bits y la comprension
...
struct BMP {
	...
	void read(const char *fname) {
		...
		if (inp) {
			...
			if(file_header.file_type != 0x4D42) { //carga  bfType con un número que representa ASCII 'M' en los bits 8-15 y ASCII 'B' en los bits 0-7
				throw std::runtime_error("Error! Unrecognized file format.");
			}
			inp.read((char*)&bmp_info_header, sizeof(bmp_info_header));
			
			// El BMPColorHeader se usa solo para imágenes transparentes
			if(bmp_info_header.bit_count == 32) {
				// Comprobar si el archivo tiene información de color de máscara de bits
				if(bmp_info_header.size >= (sizeof(BMPInfoHeader) + sizeof(BMPColorHeader))) {
					inp.read((char*)&bmp_color_header, sizeof(bmp_color_header));
					// Comprobar si los datos de píxeles se almacenan como BGRA y si el tipo de espacio de color es sRGB
			...
			// Saltar a la ubicación de los datos del píxel
			inp.seekg(file_header.offset_data, inp.beg);
			
			// Ajuste los campos de encabezado para la salida.
			// Algunos editores pondrán información adicional en el archivo de imagen, solo guardamos los encabezados y los datos.
			if(bmp_info_header.bit_count == 32) {
			...
			if (bmp_info_header.height < 0) {
				throw std::runtime_error("¡El programa solo puede tratar imágenes BMP con el origen en la esquina inferior izquierda!!");
			}
			
			data.resize(bmp_info_header.width * bmp_info_header.height * bmp_info_header.bit_count / 8);
			
			// Aquí verificamos si necesitamos tener en cuenta el relleno de filas
...
int main() {
	...
	bmp3.write("img_test_24bits11111.bmp");
